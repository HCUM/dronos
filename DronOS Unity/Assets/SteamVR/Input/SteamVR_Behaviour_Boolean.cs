...
namespace Valve.VR
{
    ...
    public class SteamVR_Behaviour_Boolean : MonoBehaviour
    {
        ...
        protected virtual void ActionUpdated(SteamVR_Action_Boolean booleanAction, SteamVR_Input_Sources inputSource, bool newValue)
        {
            if (onChange != null && booleanAction.GetChanged(inputSource))
            {
                onChange.Invoke(booleanAction, inputSource, newValue);
            }

            if (onPressDown != null && booleanAction.GetStateDown(inputSource))
            {
                onPressDown.Invoke(booleanAction, inputSource, newValue);
            }

            if (onPress != null && booleanAction.GetState(inputSource))
            {
                onPress.Invoke(booleanAction, inputSource, newValue);
            }

            if (onPressUp != null && booleanAction.GetStateUp(inputSource))
            {
                onPressUp.Invoke(booleanAction, inputSource, newValue);
            }

            if (onUpdate != null)
            {
                onUpdate.Invoke(booleanAction, inputSource, newValue);
            }
        }
    }
