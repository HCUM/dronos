...
public struct IVRInput
{
	...
	[UnmanagedFunctionPointer(CallingConvention.StdCall)]
	internal delegate EVRInputError _GetBoneName(ulong action, int nBoneIndex, [In, Out] string[] pchBoneName, uint unBufferSize);
	[MarshalAs(UnmanagedType.FunctionPtr)]
...
public class CVRInput
{
	...
	public EVRInputError GetBoneName(ulong action,int nBoneIndex,string [] pchBoneName)
	{
		EVRInputError result = FnTable.GetBoneName(action,nBoneIndex,pchBoneName,(uint) pchBoneName.Length);
		return result;
	}
